...
const server, request, seededTeams, seededAdmin;
...
describe('Team E2E', () => {
	describe('POST /team', () => {
		...
		it('should not create a team if input is invalid', async () => {
			...
			const team = { name: '' }; // The name is required
			const res = await request
				.post('/api/v1/teams')
				.set('Accept', 'application/json')
				.set('Authorization', authToken)
				.send(team);
			const errors = [ { name: 'a valid team name is required'} ];
		...
		it('should not create a team when the record already exist', async () => {
			...
			const team = { name: seededTeams[0].name };
			const res = await request
				.post('/api/v1/teams')
				.set('Accept', 'application/json')
				.set('Authorization', authToken)
	...
	//only an admin can deconste the team he created
	describe('DEconstE /team/:id', () => {

		it('should not deconste a team if not authorized', async () => {

			const teamId = seededTeams[0]._id;

			const res = await request
				.deconste(`/api/v1/teams/${teamId}`)
				.set('Accept', 'application/json');
		...
		it('should not deconste a team if id is invalid', async () => {
		...
		it('should deconste a team', async () => {
			...
			expect(res.status).toBe(200);
			expect(res.body.data).toBe('team deconsted');
		});
	});
});
